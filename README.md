...
- **快速创建**(`/guided-goal quick <一句话目标>`):跳过访谈,模型基于草稿与只读勘察自行推断五字段,推断项以「假设:…」显式标注;轮次上限按工作量估算(小 2–3 / 中 5 / 大 8–10 轮),也可显式指定数字或声明不限;
...
/guided-goal <草稿目标>               # 带草稿进入访谈,已明确字段不再重复问
/guided-goal quick <一句话目标>       # 零访谈,直接推断创建
/guided-goal quick 3 <一句话目标>     # 指定 3 轮上限
/guided-goal quick / <一句话目标>     # 不限轮次(会提醒消耗风险)
```
