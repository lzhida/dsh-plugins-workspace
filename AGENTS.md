...
dsh（deepseek harness）插件 monorepo：TypeScript + cordis（`@deepseek-ai/cordis`）插件集合，pnpm workspaces 单层结构。插件以 TS 源码形态被宿主 dsh loader 直接加载——**没有构建步骤，不产出 dist**。当前包含 `packages/dsh-guided-goal` 与 `packages/dsh-nushell-tool`。

## Architecture & Data Flow

- **加载链**：宿主 dsh loader 按插件包 `main` 字段 import TS 源码（如 `packages/dsh-guided-goal/package.json` → `main: "src/index.ts"`）→ 调用 `apply(ctx)`。
- **插件契约三符号**（见 `packages/dsh-guided-goal/src/index.ts`）：
